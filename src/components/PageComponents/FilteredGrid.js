        ...
        {postsData && postsData.length !== 0 ? (
          postsData.map((item, index) => {
            ...
            if (!Array.isArray(item.semester)) {
              item.semester = JSON.parse(item.semester);
            }
            if (!Array.isArray(item.roommate_demographics)) {
              item.roommate_demographics = JSON.parse(
                item.roommate_demographics
              );
            }

            // pretty sure this is unnecessary now because price filtering is handled serverside
