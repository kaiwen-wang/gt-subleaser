...
export function AppProvider({ children }) {
    ...
    const [maxTopPrice, setMaxTopPrice] = useState(Math.ceil(maximumDataPrice / 100) * 100);
    const [sortFormula, setSortFormula] = useState('increasingPrice');

    return (
        <AppContext.Provider value={{
            semesterPreference, setSemesterPreference,
            maxPrice, setMaxPrice,
            genderPreference, setGenderPreference,
            soloBathroomPreference, setSoloBathroomPreference,
            maxRoommates, setMaxRoommates,
            moveIn, setMoveIn,
            moveOut, setMoveOut,

            maxTopPrice, setMaxTopPrice,
            sortFormula, setSortFormula
        }}>
            {children}
        </AppContext.Provider>
    )
