...
export default function HouseDetails({ circleColors, setCircleColorsFunction }) {
    ...
    useEffect(() => {
        setPrivateBathroom(totalBathrooms >= totalBedrooms && totalBedrooms > 0 && totalBathrooms > 0);
    ...
    return (
        <>
            ...
            <span>Click circles to choose roommate gender for the sublease. Empty circle means free room.</span>
            <div className="mt-2">
                <div className="flex  overflow-x-auto  space-x-2 overflow-hidden py-2 h-12 box-border rounded border p-2 border-gray-400">
                    {circleColors.map((color, index) => (
                        <div
                            key={index}
                            className={` h-8 w-8 select-none shrink-0 rounded-full flex items-center justify-center text-white font-semibold ${color === 'O' ? "border-2 border-black" : ``} ${color === "M" ? "bg-blue-500 " : ""} ${color === "F" ? "bg-pink-500 " : ""}`
                            }
